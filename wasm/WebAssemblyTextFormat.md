...
区块指令为我们提供了独立运行某段代码块的能力，既然是代码块，我们就同时需要具备运行某些代码或者不运行某些代码的能力。这里我们就需要用到跳转指令 `br` `br_if` `br_table`，实际上，像 `loop` 指令，如果没有这些跳转指令，我们就没法实现循环，所谓的循环，无非就是代码执行一遍后，再跳到循环代码块的开头部分。所以，大部分时候区块指令必须搭配跳转指令才能实现它要实现的代码逻辑。另外，跳转指令都具备跳转到某一个区块之外的能力，这里的`之外`表示，它会跳转到对应的区块指令的 `end` 之后再继续执行，而不会再执行它（跳转指令）之后的其它指令。
